...
<body>
  ...
  <div class="container">
    <div class="row">
      <!-- Le profil de celui qui est connecter -->
        ...
        <table class="table table-sm  table-bordered">
          ...
          <tbody>
            <tr>
              ...
              <td><a href="catDepense.php" class="btn btn-success text-center">Catégirie dépanse</a></td>
            </tr>
            <tr>
    ...
    <!-- Les categorie des travailleurs -->
    <div class="row">
      <div class="bd-example fon text-center col-lg-5">
        <table class="table table-sm  table-bordered">
          <thead>
            <tr>
              <div class="titre bg-success text-white">
                Liste des categories travailleurs
              </div>
              <th>N°</th>
              <th>categorie</th>
              <th>Actions</th>
      ...
      <!-- Liste des categories de charge -->
    ...
    <div class="row">
      ...
      <div class="bd-example fon text-center col-lg-7">
        <table class="table table-sm  table-bordered">
          ...
          <tbody>
            <?php
            $getRecette = $bdd->prepare("SELECT recette.dateR, travailleur.nom, travailleur.postnom , recette.libelle,recette.montant FROM recette,  travailleur WHERE travailleur.id=recette.idT AND recette.statut=? AND recette.etat=? ORDER BY recette.id DESC ");
            $getRecette->execute(array($statut, $etat));
            $numero = 0;
      ...
      <div class="bd-example fon text-center col-lg-12">
        <table class="table table-sm table-bordered">
          ...
          <tbody>
            <tr>
              <?php
              $getDepense = $bdd->prepare("SELECT depenses.dateD, depenses.libelle, depenses.details, depenses.montant, travailleur.nom, travailleur.postnom, categoried.categorie FROM depenses, categoried, travailleur WHERE travailleur.id=depenses.codeT AND categoried.codeCat=depenses.codeCatDepanse AND depenses.statut=? AND depenses.etat=? ORDER BY depenses.id DESC");
              $getDepense->execute(array($statut, $etat));
              $numero = 0;
