...
class Depense extends Model
{
    ...
    public function update(int$id,array$d):void{if(!$this->isOpen($id))throw new RuntimeException('Une dépense clôturée ne peut pas être modifiée.');$this->db->prepare('UPDATE depenses SET affection_vehicule=?,categorie_depense_id=?,date_depense=?,libelle=?,montant=?,devise=? WHERE id=? AND cloturer=0')->execute([$d['affection_vehicule'],$d['categorie_depense_id'],$d['date_depense'],$d['libelle'],$d['montant'],$d['devise'],$id]);}
    public function delete(int$id):void{if(!$this->isOpen($id))throw new RuntimeException('Une dépense clôturée ne peut pas être supprimée.');$this->db->prepare('DELETE FROM depenses WHERE id=? AND cloturer=0')->execute([$id]);}
    private function isOpen(int$id):bool{$s=$this->db->prepare('SELECT COUNT(*) FROM depenses WHERE id=? AND cloturer=0');$s->execute([$id]);return(int)$s->fetchColumn()===1;}
}
